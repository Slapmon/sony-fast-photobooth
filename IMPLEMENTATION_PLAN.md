...
**Done when:** single photo, end to end, on the Pi, with a waterfall showing every millisecond. **Status: 14/14 done.** Full capture loop (arm → countdown → capture → real JPEG served → review → dismiss) verified end to end on the dev laptop against the mock backend, with `/debug/traces`/`/debug/timings` verified live against real capture data. Real capture *timing* was independently proven on the Pi in Phase 0 (§6) via `tools/cam_test.py`.

**Gap closed (2026-09-03):** the FastAPI app was run end to end on the Pi against the real `GphotoBackend` for the first time — both single-photo and 4-shot collage modes, real Sony body, real files. It immediately surfaced a real bug: `GphotoBackend.trigger_capture()` returned the raw PTP folder/filename (`"{folder}/{name}"`) as `capture_id`, which every other caller (`web/session.py`, admin's test-shot) treats as a flat filename component — a value containing `/` broke `Path(...) / f"{capture_id}.jpg"`. Fixed by generating a plain `uuid.uuid4().hex` instead, keeping the real PTP path in the existing internal `_pending` dict. Also found and fixed a startup-robustness bug in `web/app.py`'s `lifespan`: an exception raised after spawning the camera-worker subprocess but before reaching the `try: yield` block (e.g. a misconfigured `CupsBackend`) leaked that subprocess, orphaning it on `camera.worker_port` and blocking the next boot — restructured into nested try/finally so every resource is cleaned up regardless of where startup fails. Deployed to the Pi (`~/photobooth`, running as `admin` under systemd — see `deploy/systemd/photobooth.service`'s note that this is a testing-appropriate deviation from the dedicated-`photobooth`-user setup the unit file itself documents; migrate before a real event for the extra isolation).
